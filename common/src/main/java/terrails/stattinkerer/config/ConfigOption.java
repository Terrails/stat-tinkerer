...
public class ConfigOption<C, O> implements Supplier<O> {
    ...
    public static <C, O> ConfigOption<List<C>, List<O>> defineList(String path, String comment, List<C> defaultValues, Predicate<Object> validator, Function<O, C> serializer, Function<C, O> deserializer) {
        return new ConfigOption<>(path, comment, defaultValues,
                o -> {
                ...
                values -> values.stream().map(serializer).toList(), values -> values.stream().map(deserializer).toList()
        );
    }

    ConfigOption(String path, String comment, C defaultValue, Predicate<Object> optionValidator, Function<O, C> serializer, Function<C, O> deserializer) {
        this.path = path;
