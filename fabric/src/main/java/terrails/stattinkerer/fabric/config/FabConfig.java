package terrails.stattinkerer.fabric.config;
...
public class FabConfig extends CommentedConfigWrapper<CommentedFileConfig> implements CommentedFileConfig {
    ...
    public FabConfig(Object... instances) {
        super(CommentedFileConfig
                .builder(FabricLoader.getInstance().getConfigDir().resolve(CStatTinkerer.MOD_ID + ".toml"), TomlFormat.instance())
    ...
    @Override
    public void save() {
        LOGGER.info("Saving {} config file.", getFile().getName());
        this.config.save();
        LOGGER.debug("Saved {} config file.", getFile().getName());
    }

    @Override
    public void load() {
        LOGGER.info("Loading {} config file", getFile().getName());
        this.loadFile();
        if (this.firstLoad) {
        ...
        LOGGER.debug("Loaded {} config file", getFile().getName());
    }
    ...
    private void loadFile() {
        while (true) {
            try {
                ...
                this.disableWatcher();
                String deformedFile = CStatTinkerer.MOD_ID + "-" + LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd_HH.mm.ss")) + "-deformed.toml";
                try {
        ...
        int corrections = this.spec.correct(this.config, (action, pathList, incorrectValue, correctedValue) -> {
            ...
            switch (action) {
                ...
                case REMOVE -> LOGGER.warn("Invalid entry {} removed from {}", path, getFile().getName());
                case REPLACE -> LOGGER.warn("Invalid entry {}: value {} replaced by {} in {}", path, incorrectValue, correctedValue, getFile().getName());
            }
        });
        ...
        if (modified) {
            LOGGER.debug("Saving corrected {} config file", getFile().getName());
            this.save();
        }
    }
